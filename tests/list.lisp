...
(test (equal (let ((x (list (1 2) 3))) (setf (caar x) 0) x) '((0 2) 3)))
(test (equal (let ((x (list 1 2 3))) (setf (cadr x) 0) x) '(1 0 3)))
(test (equal (let ((x (list (1 2) 3))) (setf (cdar x) 0) x) '((1 . 0) 3)))
(test (equal (let ((x (list 1 2 3))) (setf (cddr x) '(0)) x)  '(1 2 0)))
(test (equal (let ((x (list ((1))))) (setf (caaar x) 0) x) '(((0)))))
(test (equal (let ((x (list 1 (2)))) (setf (caadr x) 0) x) '(1 (0))))
(test (equal (let ((x (list (1 2)))) (setf (cadar x) 0) x) '((1 0))))
(test (equal (let ((x (list 1 2 3))) (setf (caddr x) 0) x) '(1 2 0)))
(test (equal (let ((x (list ((1 2))))) (setf (cdaar x) '(0)) x) '(((1 0)))))
(test (equal (let ((x (list 1 (2 3)))) (setf (cdadr x) '(0)) x) '(1 (2 0))))
(test (equal (let ((x (list (1 2 3)))) (setf (cddar x) '(0)) x) '((1 2 0))))
(test (equal (let ((x (list 1 2 3 4))) (setf (cdddr x) '(0)) x) '(1 2 3 0)))
(test (equal (let ((x (list (((1)))))) (setf (caaaar x) 0) x) '((((0))))))
(test (equal (let ((x (list 1 ((2))))) (setf (caaadr x) 0) x) '(1 ((0)))))
(test (equal (let ((x (list (1 (2))))) (setf (caadar x) 0) x) '((1 (0)))))
(test (equal (let ((x (list 1 2 (3)))) (setf (caaddr x) 0) x) '(1 2 (0))))
(test (equal (let ((x (list ((1 2))))) (setf (cadaar x) 0) x) '(((1 0)))))
(test (equal (let ((x (list 1 (2 3)))) (setf (cadadr x) 0) x) '(1 (2 0))))
(test (equal (let ((x (list (1 2 3)))) (setf (caddar x) 0) x) '((1 2 0))))
(test (equal (let ((x (list 1 2 3 4))) (setf (cadddr x) 0) x) '(1 2 3 0)))
(test (equal (let ((x (list (((1 2)))))) (setf (cdaaar x) '(0)) x) '((((1 0))))))
(test (equal (let ((x (list 1 ((2 3))))) (setf (cdaadr x) '(0)) x) '(1 ((2 0)))))
(test (equal (let ((x (list (1 (2 3))))) (setf (cdadar x) '(0)) x) '((1 (2 0)))))
(test (equal (let ((x (list 1 2 (3 4)))) (setf (cdaddr x) '(0)) x) '(1 2 (3 0))))
(test (equal (let ((x (list ((1 2 3))))) (setf (cddaar x) '(0)) x) '(((1 2 0)))))
(test (equal (let ((x (list 1 (2 3 4)))) (setf (cddadr x) '(0)) x) '(1 (2 3 0))))
(test (equal (let ((x (list (1 2 3 4)))) (setf (cdddar x) '(0)) x)  '((1 2 3 0))))
