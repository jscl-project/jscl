;;; loop.lisp --- Implementation of the LOOP macro
...
;;; 
;;; This file was actually taken from ai.mit.edu:pub/loop-macro.tar
;;;
...
;;; [gsb@palladian] 30-oct-86 18:23  don't generate (type notype var) decls, 
;;;                                  special-case notype into T. (The NOTYPE
;;;                                  type keyword needs to be around for
;;;                                  compatibility.)
;;; [gsb@palladian] 30-oct-86 18:48  bogus case clause in loop-do-collect.  
;;;                                  Syntax:common-lisp in file attribute
;;;		                     list, for symbolics gratuitousness.
...

;;; ********************************
;;; Begin Local Change Log *********
;;; ********************************
;;; The changes noted below are in the version maintained by mkant@cs.cmu.edu.
;;;
;;; Bob Bane [JRB] <bane%rattler@cs.UMD.EDU> 11/02/90
;;; Changes to bring loop.lisp in closer conformance with CLtL2:
;;;  Changed loop-optional-type to allow optional of-type keyword.
;;;  Changed LOOP-DO-COLLECT to accept
;;;    <collect-operator> <expr> [into <rvar>] [<type-spec>]
;;;    the old code accepted
;;;    <collect-operator> <expr> [<type-spec>] [into <rvar>]
;;;
;;; The following change in the version from rascal.ics.utexas.edu doesn't
;;; seem to be necessary anymore, so it isn't implemented here.
;;; <boyer@rascal.ics.utexas.edu> 7/9/87
;;; In define-loop-macro moved &environment and its formal to right after
;;; &whole to overcome bug in KCL. This replaces
;;; (&whole whole-form &rest keywords-and-forms &environment env) with
;;; (&whole whole-form &environment env &rest keywords-and-forms).


;;; ********************************
;;; End Local Change Log ***********
;;; ********************************
...
(defun loop-optional-type ()
  (let ((token (car loop-source-code)))
    ;; JRB this should add of-type (and seems to be the place to do it)
    (if (loop-tequal token 'of-type)
  ...
  (let ((var nil) (form nil) (tem nil) (tail nil) (dtype nil) (cruft nil) (rvar nil)
	(ctype (case type
		 ...
		 (t (error "LOOP internal error:  ~S is an unknown collecting keyword."
			   type)))))
    ;; JRB This used to parse <form> [<type-spec>] [into <rvar>]
    ;; where the book says <form> [into <rvar>] [<type-spec>]
    ;; At least the fix was easy!
    (setq form (loop-get-form type))
    (cond ((loop-tequal (car loop-source-code) 'into)
	   (loop-pop-source)
	   (setq rvar (setq var (loop-pop-source)))))
    (setq dtype (loop-optional-type))
